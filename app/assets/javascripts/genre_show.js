$( document ).on('turbolinks:load', function() {
  ...
  class Genres {
    ...
    pairAuthors(genreInfo) {
      const bookAuthorPairs = []
      const authorsArray = genreInfo.authors;
      let authorName;
      // debugger
      genreInfo.books.forEach(book => {
        authorName = this.authorMatch(book.author_id, authorsArray);
          bookAuthorPairs.push([book.title, authorName, book.id, book.author_id])
      })
      this.showGenreInfo(bookAuthorPairs)
    }

    renderTr(bookAuthorPair) {
      // debugger
      return `<tr><td><a href="http://localhost:3000/books/${bookAuthorPair[2]}">${bookAuthorPair[0]}</a></td><td><a href="http://localhost:3000/authors/${bookAuthorPair[3]}">${bookAuthorPair[1]}</a></td></tr>`
    }

    showGenreInfo(bookAuthorPairs) {
      const genreShowTable = $("#genre_show_table");
      const tableHeader = `<th>Book Title</th> <th>Author</th>`;
      // debugger
      const genreBooksString = bookAuthorPairs.map(pair => this.renderTr(pair)).join('')
      // debugger
      // const tableContents = jQuery.parseHTML(tableHeader + genreBooksString);
